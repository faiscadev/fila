...
- [ ] Task 1: Extend Metrics struct with throttle and Lua instruments (AC: #1, #2, #3, #4, #5)
  - [ ] Subtask 1.1: Add `throttle_decisions: Counter<u64>` for `fila.throttle.decisions`
  - [ ] Subtask 1.2: Add `throttle_tokens_remaining: Gauge<f64>` for `fila.throttle.tokens_remaining`
  - [ ] Subtask 1.3: Add `lua_execution_duration: Histogram<f64>` for `fila.lua.execution_duration_us`
  - [ ] Subtask 1.4: Add `lua_errors: Counter<u64>` for `fila.lua.errors`
  - [ ] Subtask 1.5: Add `lua_circuit_breaker_activations: Counter<u64>` for `fila.lua.circuit_breaker.activations`
  - [ ] Subtask 1.6: Add recording methods for each new metric
- [ ] Task 2: Add LuaExecOutcome to surface execution result to scheduler (AC: #3, #4, #5)
  - [ ] Subtask 2.1: Define `LuaExecOutcome` enum in `lua/mod.rs`: `Success`, `ScriptError { circuit_breaker_tripped: bool }`, `CircuitBreakerBypassed`
  - [ ] Subtask 2.2: Change `run_on_enqueue` return type from `Option<OnEnqueueResult>` to `Option<(OnEnqueueResult, LuaExecOutcome)>`
  - [ ] Subtask 2.3: Change `run_on_failure` return type from `Option<OnFailureResult>` to `Option<(OnFailureResult, LuaExecOutcome)>`
  - [ ] Subtask 2.4: Update all call sites in scheduler to destructure the new return type
- [ ] Task 3: Instrument throttle decisions in DRR delivery path (AC: #1)
  - [ ] Subtask 3.1: After `peek_keys` returns false (throttled), record `hit` for each throttle key
  - [ ] Subtask 3.2: After `consume_keys` on successful delivery, record `pass` for each throttle key
- [ ] Task 4: Instrument throttle gauge in `record_gauges` (AC: #2)
  - [ ] Subtask 4.1: Iterate `throttle.key_stats()` and record `tokens_remaining` per throttle key
- [ ] Task 5: Instrument Lua execution in scheduler (AC: #3, #4, #5)
  - [ ] Subtask 5.1: Time `run_on_enqueue` with `Instant::now()`, record duration histogram (only when script actually ran, not on CB bypass)
  - [ ] Subtask 5.2: Time `run_on_failure` with `Instant::now()`, record duration histogram
  - [ ] Subtask 5.3: Record `lua_errors` counter on `ScriptError` outcome
  - [ ] Subtask 5.4: Record `lua_circuit_breaker_activations` on `circuit_breaker_tripped = true`
- [ ] Task 6: Extend test harness for histogram assertions (AC: #7)
  - [ ] Subtask 6.1: Add `assert_histogram` helper that checks count and optionally sum
  - [ ] Subtask 6.2: Add `assert_gauge_f64_single` helper for single-label f64 gauge (throttle tokens)
- [ ] Task 7: Integration tests (AC: #7)
  - [ ] Subtask 7.1: Test throttle_decisions counter records pass/hit per key
  - [ ] Subtask 7.2: Test throttle_tokens_remaining gauge records current tokens
  - [ ] Subtask 7.3: Test lua_execution_duration histogram records entries
  - [ ] Subtask 7.4: Test lua_errors counter increments on script failure
  - [ ] Subtask 7.5: Test lua_circuit_breaker_activations counter increments on trip
