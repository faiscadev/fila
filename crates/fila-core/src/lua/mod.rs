...
impl LuaEngine {
    ...
    pub fn new(
        ...
        bridge::register_fila_api(&lua, storage, partition)
            .map_err(LuaError::BridgeRegistration)?;
...
mod tests {
    ...
    fn test_engine() -> (LuaEngine, tempfile::TempDir) {
        ...
        let engine = LuaEngine::new(storage, PartitionId::DEFAULT, &lua_config).unwrap();
        (engine, dir)
    }
