...
/// Each node has distinct client (binary protocol) and cluster ports.
...
impl TestCluster {
    ...
    /// Get the binary protocol address of node `i` (e.g., "127.0.0.1:12345").
    /// Used by SDK and CLI connections.
    pub fn addr(&self, i: usize) -> &str {
        &self.nodes[i].as_ref().expect("node not running").addr
    }

    /// Get the binary protocol address of node `i`.
...
fn spawn_and_wait(data_dir: tempfile::TempDir, client_port: u16) -> ClusterNode {
    ...
    assert!(
        ...
        "cluster node did not become reachable at {addr} within 15s"
    );
