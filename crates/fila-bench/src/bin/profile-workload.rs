...
/// Designed to be run alongside a profiler (dtrace, perf, sample).
///
/// Environment variables:
///   PROFILE_WORKLOAD    enqueue-only|consume-only|lifecycle (default: enqueue-only)
...
///   PROFILE_SERVER_ADDR if set, connect to this server instead of starting one
#[tokio::main]
async fn main() {
    ...
    let _server;
    let connect_addr = match std::env::var("PROFILE_SERVER_ADDR") {
        Ok(a) => a,
        Err(_) => {
    ...
    match workload.as_str() {
        "enqueue-only" => {
            run_enqueue_only(&connect_addr, queue_name, &payload, concurrency, duration).await;
        }
        "consume-only" => {
            run_consume_only(&connect_addr, queue_name, &payload, concurrency, duration).await;
        }
        "lifecycle" => {
            run_lifecycle(&connect_addr, queue_name, &payload, concurrency, duration).await;
        }
        other => {
...
async fn connect(addr: &str) -> FilaClient {
    FilaClient::connect(addr).await.expect("connect to fila-server")
}
