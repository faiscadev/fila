...
use crate::error::{message_status_error, queue_status_error, ClientError};
...
impl FilaClient {
    ...
    pub async fn connect_with_options(options: ConnectOptions) -> Result<Self, ClientError> {
    ...
    pub async fn lease(
        &self,
        queue: &str,
    ) -> Result<impl Stream<Item = Result<LeaseMessage, ClientError>>, ClientError> {
        let response = self
            .inner
            .clone()
            .lease(LeaseRequest {
        ...
        let stream = response.into_inner().filter_map(|result| match result {
            ...
            Err(status) => Some(Err(queue_status_error(status))),
        });
    ...
    pub async fn ack(&self, queue: &str, message_id: &str) -> Result<(), ClientError> {
        self.inner
            .clone()
            .ack(AckRequest {
                queue: queue.to_string(),
